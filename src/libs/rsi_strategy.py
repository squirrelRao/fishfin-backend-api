    ...
    def run(self,user_id,quote_currency,base_currency,period,ktime,limit_trade_count,trade_name="simulation"):
        ...
        if  trade is None:
            ...
            if action in ["buy","sell"]:
                ...
                trade.submit_market_transaction(user_id,period,symbol,amount,cur_price,quote_currency,trans_fee,ktime,self.name,action,rsi)
            else:
                ...
                trade.log(user_id,"keep_log",0,cur_price,period,symbol,quote_currency,trans_fee,base_currency_balance,quote_currency_balance,ktime,self.name,action="finish",log_id="",rsi=rsi)
